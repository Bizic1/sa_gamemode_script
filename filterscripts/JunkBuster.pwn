...
PRIVATE: JB::MutePlayer (playerid, time, reason [])//Time is seconds.
{
	if (IsPlayerConnected (playerid) && time)
	{
		...
		JB::SendFormattedMessageToAll (ANTICHEAT, "(Anticheat):{FFFFFF} Mutiram igraca '%s' (%d) na %d sekundi. "RBojaa"Razlog: %s", JB::PlayerInfo [playerid][JB::pName], playerid, time, reason);
		return 1;
	}
	return 0;
}
...
PRIVATE: JB::Kick (playerid, reason [])
{
	if (IsPlayerConnected (playerid) && !IsPlayerNPC (playerid))
	{
		...
		JB::SendFormattedMessageToAll (ANTICHEAT, "(Anticheat):{FFFFFF} Kikujem igraca '%s'. "RBojaa"Razlog: %s", JB::PlayerInfo [playerid][JB::pName], reason);
...
PRIVATE: JB::Ban (playerid, reason [])
{
	if (IsPlayerConnected (playerid) && !IsPlayerNPC (playerid))
	{
		...
		TogglePlayerControllable (playerid, false);
		SetCameraBehindPlayer (playerid);
		JB::SendFormattedMessageToAll (ANTICHEAT, "(Anticheat):{FFFFFF} Banujem igraca '%s'. "RBojaa"Razlog: %s", JB::PlayerInfo [playerid][JB::pName], reason);
...
PRIVATE: DisplayKickBanText (playerid, reason [], bool: ban)
{
	if (IsPlayerConnected (playerid))
	{
		...
		format (string, sizeof (string), "~y~Name:~n~~w~%s~n~~y~Date:~n~~w~%s~n~~y~Time:~n~~w~%s~n~~y~IP:~n~~w~%s~n~~y~Razlog:~n~~w~%s", JB::PlayerInfo [playerid][JB::pName], JB::GetDate (), JB::GetTime (),
...
PUBLIC: CheckText (playerid, text []) //return 1: something forbidden found, return 0: everything OK!
{
	...
	{
		SendClientMessage (playerid, ANTICHEAT, "(Anticheat):{FFFFFF} You are not allowed to use Caps Lock! Press [Caps Lock] to disable it.");
		return 1;
	}
	...
	{
		SendClientMessage (playerid, ANTICHEAT, "(Anticheat):{FFFFFF} You are not allowed to use such words!");
		return 1;
	}
	return 0;
}
...
PRIVATE: JunkBusterReport (playerid, report [], details [])
{
	if (IsPlayerConnected (playerid))
	{
		...
		format (string, sizeof (string), "(Anticheat):{FFFFFF} Sumnjam na igraca %s: %s (%d, %s), %s.", report, JB::PlayerInfo [playerid][JB::pName], playerid, JB::PlayerInfo [playerid][JB::pIp], details);
		ForEachPlayer (i)
			if (JB::IsPlayerAdmin (i) && i != playerid)
				SendClientMessage (i, ANTICHEAT, string);
			...
			if (!JB::IsPlayerAdmin (i) && IsPlayerInValidState (i) && !DOB::GetBit (JB::AntiBugKilled, i)) //!JB::PlayerInfo [i][JB::pAntiBugKilled])
			{
			    if (JB::Variables [JOYPAD] && !JB::PlayerInfo [i][JB::pFreezeTime])
				{
				    ...
				    if((updown != 128 && updown != 0 && updown != -128) || (leftright != 128 && leftright != 0 && leftright != -128))
				    {
						...
						else
						{
						    ...
						    SetSyncTime (i, SYNC_TYPE_VEHICLE);
							TogglePlayerControllable (i, false);
							SendClientMessage (i, ANTICHEAT, "(Anticheat):{FFFFFF} Molimo vas prestanite koristiti joypad/gamepad.");
						}
				    }
				}
				...
				if (JB::Variables [MIN_FPS])
				{
					...
					if (var < JB::Variables [MIN_FPS])
					{
						...
						{
							JB::SendFormattedMessage (i, ANTICHEAT, "(Anticheat):{FFFFFF} Please fix your framerate (FPS) or you will get kicked! (Min %d, your FPS: %d)", JB::Variables [MIN_FPS], var);
							SendClientMessage (i, ANTICHEAT, "(Anticheat):{FFFFFF} Pressing F7 once may help to fix it. (Removes the outlines of the letters in chat window.)");
						}
					}
				}
				...
				if (IsWeaponForbiddenForPlayer (i, var))
				{
					...
					format (reason, sizeof (reason), "Koristi citeve za oruzje (%s)", reason);
					JB::Ban (i, reason);
					continue;
				}
	...
	PUBLIC: UnbanIp (ipstring [])
	{
		...
		for (new i = 0; i < IpBanCount; ++i)
		{
			if (IsSameIp (IpBans [i], ip))
			{
				...
				JB::LogEx ("IP %s je unbanovan.", ipstring);
				SaveIpBans ();
				return 1;
			}
		}
		...
		for (new i = 0; i < TempBanCount; ++i)
		{
			if (!strcmp (TempBanInfo [i][tbName], JB::PlayerInfo [playerid][JB::pName], false))
			{
				if (TempBanInfo [i][tbTime] > time)
				{
					...
					JB::SendFormattedMessage (playerid, ANTICHEAT, "(Anticheat):{FFFFFF} You are temporary for %d day(s), %d hour(s), %d minute(s) and %d second(s)!", days, hours, minutes, seconds);
...
CALLBACK: OnPlayerReport (playerid, reporterid, report [])
{
	...
	JB::LogEx ("%s (%s) has reported %s (%s). "RBojaa"Razlog: %s", JB::PlayerInfo [reporterid][JB::pName], JB::PlayerInfo [reporterid][JB::pIp], JB::PlayerInfo [playerid][JB::pName], JB::PlayerInfo [playerid][JB::pIp], report);
		    ...
		    if (JB::PlayerInfo [killerid][JB::pCurrentState] == PLAYER_STATE_DRIVER)
		    {
				if (JB::Variables [DRIVE_BY] >= 2 && (reason == WEAPON_UZI || reason == WEAPON_MP5 || reason == WEAPON_TEC9 || reason == 50)) // 50 = helicopter blades.
				{
					...
					else
					{
						...
						SendClientMessage (killerid, ANTICHEAT, "(Anticheat):{FFFFFF} You have been killed for drive-by!");
					}
				}
			 	...
			 	if (JB::Variables [ARMED_VEHICLES])
			 	{
					...
					if (IsArmedVehicle (vehicleid))
					{
					    if (++JB::Warnings [killerid]{ARMED_VEHICLES} >= JB::Variables [ARMED_VEHICLES])
						{
							...
							JB::SendFormattedMessage (killerid, ANTICHEAT, "(Anticheat):{FFFFFF} Killing with a %s is not allowed! Stop it! (Warning %d/%d)", JB::GetVehicleName (vehicleid), JB::Warnings [killerid]{ARMED_VEHICLES}, JB::Variables [ARMED_VEHICLES]);
					}
			 	}
			}
			...
			if (JB::Variables [SPAWNKILL])
			{
				if (JB::PlayerInfo [playerid][JB::pSpawnKillProtected])
				{
					if (++JB::Warnings [killerid]{SPAWNKILL} >= JB::Variables [SPAWNKILL])
						...
						JB::SendFormattedMessage (killerid, ANTICHEAT, "(Anticheat):{FFFFFF} Do not spawnkill! (Warning %d/%d)", JB::Warnings [killerid]{SPAWNKILL}, JB::Variables [SPAWNKILL]);
				}
		 	}
		}
	...
	if (!IsPlayerNPC (playerid))
	{
	        ...
	        if (JB::Variables [BLACKLIST])
	        {
		        ...
		        if (banned)
		        {
		            ...
		            {
		                SendClientMessage (playerid, ANTICHEAT, "(Anticheat):{FFFFFF} You are banned from this server!");
						JB::LogEx ("%s (%s) has been banned for Ban evading.", JB::PlayerInfo [playerid][JB::pName], JB::PlayerInfo [playerid][JB::pIp]);
					...
					else if (t < unbantime)
					{
					    ...
					    JB::SendFormattedMessageToAll (ANTICHEAT, "(Anticheat):{FFFFFF} You are temporary for %d day(s), %d hour(s), %d minute(s) and %d second(s)!", days, hours, minutes, seconds);
...
CALLBACK: OnPlayerText (playerid, text [])
{
	...
	{
		JB::SendFormattedMessage (playerid, ANTICHEAT, "(Anticheat):{FFFFFF} Niste u mogucnosti da koristite chat na %d sekundi zato sto ste mutirani!", JB::PlayerInfo [playerid][JB::pMuted]);
		return 0;
	}
...
//==============================================================================
	...
	COMMAND:oputest (playerid, params [])
	{
		...
		t = GetTickCount () - t;
		JB::SendFormattedMessage (playerid, ANTICHEAT, "OnPlayerUpdate time: %d ms", t);
		return 1;
	}
	...
	COMMAND:jbtest (playerid, params [])
	{
		...
		t = GetTickCount () - t;
		JB::SendFormattedMessage (playerid, ANTICHEAT, "JunkBuster time: %d ms", t);
		return 1;
	}
...
COMMAND:myfps (playerid, params [])
{
	...
	if (!JB::Variables [MIN_FPS])
		return SendClientMessage (playerid, ANTICHEAT, "This function has been disabled!");
...
COMMAND:jbchrome (playerid, params [])
{
	...
	if (JB::sscanf (params, "u", id))
		return SendClientMessage (playerid, ANTICHEAT, "Usage:{D6D6D6} /jbchrome <ID/name>");
		
	if (JunkBusterChrome (id, JB::PlayerInfo [playerid][JB::pName]))
		JB::SendFormattedMessage (playerid, JB_GREEN, "(Anticheat):{FFFFFF} JunkBuster Chrome has successfully collected data of player '%s'!", JB::PlayerInfo  [id][JB::pName]);
	else
	    JB::SendFormattedMessage (playerid, ANTICHEAT, "(Anticheat):{FFFFFF} JunkBuster Chrome could not colect data of player '%s'!", params);
	return 1;
}
...
COMMAND:jbsethomepage (playerid, params [])
{
	...
	if (isnull (params))
	    return SendClientMessage (playerid, ANTICHEAT, "Usage:{D6D6D6} /jbsethomepage <homepage>");
	    
	strcpy (JB::Homepage, params);
    DOF2::SetString (CONFIG_FILE, "Homepage", JB::Homepage, "Misc");
    JB::SendFormattedMessage (playerid, JB_GREEN, "(Anticheat):{FFFFFF} Homepage has been set to '%s'!", params);
	return 1;
}
...
COMMAND:blackadd (playerid, params [])
{
	...
	if (JB::sscanf (params, "u", id))
		return SendClientMessage (playerid, ANTICHEAT, "Usage:{D6D6D6} /blackadd <ID/name>");
	...
	if (!IsPlayerConnected (id))
	{
		...
		#endif
				JB::SendFormattedMessage (playerid, JB_GREEN, "(Anticheat):{FFFFFF} Player '%s' has successfully been added to blacklist!", params);
			else
				JB::SendFormattedMessage (playerid, ANTICHEAT, "(Anticheat):{FFFFFF} Could not add player '%s' to blacklist!", params);
	}
	else
	{
		if (id != playerid)
		{
		        ...
		        {
		            JB::SendFormattedMessage (playerid, JB_GREEN, "(Anticheat):{FFFFFF} Player '%s' has successfully been added to blacklist!", JB::PlayerInfo [id][JB::pName]);
				...
				if (AddPlayerToBlacklist (id))
					...
					JB::SendFormattedMessage (playerid, ANTICHEAT, "(Anticheat):{FFFFFF} Could not add player '%s' to blacklist!", JB::PlayerInfo [id][JB::pName]);
		}
	}
	return 1;
}
...
COMMAND:blackdel (playerid, params [])
{
	...
	if (!isnull (params))
	{
		...
		#endif
				JB::SendFormattedMessage (playerid, JB_GREEN, "(Anticheat):{FFFFFF} Player '%s' has successfully been removed from blacklist!", params);
			else
				JB::SendFormattedMessage (playerid, ANTICHEAT, "(Anticheat):{FFFFFF} Could not remove player '%s' from blacklist!", params);
			return 1;
	}
	return SendClientMessage (playerid, ANTICHEAT, "Usage:{D6D6D6} /blackdel <name>");
}
...
COMMAND:whiteadd (playerid, params [])
{
	...
	if (!isnull (params))
	{
		...
		#endif
				JB::SendFormattedMessage (playerid, JB_GREEN, "(Anticheat):{FFFFFF} Player '%s' has successfully been added to whitelist!", params);
			else
				JB::SendFormattedMessage (playerid, ANTICHEAT, "(Anticheat):{FFFFFF} Could not add player '%s' to whitelist!", params);
			return 1;
	}
	return SendClientMessage (playerid, ANTICHEAT, "Usage:{D6D6D6} /whiteadd <name>");
}
...
COMMAND:whitedel (playerid, params [])
{
	...
	if (!isnull (params))
	{
		...
		#endif
				JB::SendFormattedMessage (playerid, JB_GREEN, "(Anticheat):{FFFFFF} Player '%s' has successfully been removed from whitelist!", params);
			else
				JB::SendFormattedMessage (playerid, ANTICHEAT, "(Anticheat):{FFFFFF} Could not remove player '%s' from whitelist!", params);
			return 1;
	}
	return SendClientMessage (playerid, ANTICHEAT, "Usage:{D6D6D6} /whitedel <name>");
}
...
COMMAND:jbsetvar (playerid, params [])
{
	...
	if (JB::sscanf (params, "si", var, value))
		return SendClientMessage (playerid, ANTICHEAT, "Usage:{D6D6D6} /jbsetvar <variable name> <0/1(/max ping)> ");

	if (SetJBVar (var, value))
	{
	    JB::SendFormattedMessage (playerid, JB_GREEN, "(Anticheat):{FFFFFF} JunkBuster variable '%s' has successfully been set to %d.", var, value);
...
COMMAND:tban (playerid, params [])
{
	...
	if (JB::sscanf (params, "uiz", id, days, reason))
		return SendClientMessage (playerid, ANTICHEAT, "Usage:{D6D6D6} /tban <ID/name> <days> <reason>");
...
COMMAND:tunban (playerid, params [])
{
	...
	if (!isnull (params))
	{
		...
		#endif
				JB::SendFormattedMessage (playerid, JB_GREEN, "(Anticheat):{FFFFFF} Temporary ban of player '%s' has successfully been deleted.", params);
			else
				JB::SendFormattedMessage (playerid, ANTICHEAT, "(Anticheat):{FFFFFF} Could not delete temporary ban of player '%s'!", params);
			return 1;
	}
	return SendClientMessage (playerid, ANTICHEAT, "Usage:{D6D6D6} /tunban <name>");
}

COMMAND:jbbanip (playerid, params [])
{
	if (!IsPlayerAdmin (playerid))
		return 0;
	    ...
	    SendClientMessage (playerid, ANTICHEAT, "(Anticheat):{FFFFFF} Sorry, this command is currently not available when using JunkBuster database.");
	    return 1;
	#else
		if (!isnull (params))
		{
			...
			{
				if (BanIp (params))
					JB::SendFormattedMessage (playerid, JB_GREEN, "(Anticheat):{FFFFFF} IP %s has successfully been banned!", params);
				else
					JB::SendFormattedMessage (playerid, ANTICHEAT, "(Anticheat):{FFFFFF} Could not ban IP %s!", params);
				return 1;
			}
		}
		return SendClientMessage (playerid, ANTICHEAT, "Usage:{D6D6D6} /banIP <IP> ");
	#endif
}

COMMAND:jbunbanip (playerid, params [])
{
	if (!IsPlayerAdmin (playerid))
		return 0;
	    ...
	    if (JBDB::UnbanIp (params))
			JB::SendFormattedMessage (playerid, JB_GREEN, "(Anticheat):{FFFFFF} IP %s has successfully been unbanned!", params);
		else
			JB::SendFormattedMessage (playerid, ANTICHEAT, "(Anticheat):{FFFFFF} Could not unban IP %s!", params);
		return 1;
	#else
		if (!isnull (params))
		{
		...
		return SendClientMessage (playerid, ANTICHEAT, "Usage:{D6D6D6} /unbanIP <IP> ");
	#endif
}
				...
				case 24, 25, 27, 29, 30, 31, 33, 34: // Deagle, Shotung, SPAS12, MP5, AK47, M4, Rifle, Sniper Rifle
				{
					...
					else if (((oldkeys & KEY_FIRE) || (oldkeys == KEY_FIRE)) && ((newkeys & KEY_CROUCH) || (newkeys == KEY_CROUCH)) && (GetTickCount () - JB::PlayerInfo [playerid][JB::pFired]) < 750)
					{
						if (++JB::Warnings [playerid]{CBUG} & 1 == 0)
							SendClientMessage (playerid, ANTICHEAT, "(Anticheat):{FFFFFF} Molimo vas prestanite koristiti C/Bug bicete kikovani/banovani.");
	...
	if (1 <= JB::Variables [DRIVE_BY] <= 2 && PRESSED (KEY_FIRE) && (HOLDING (KEY_LOOK_RIGHT) || HOLDING (KEY_LOOK_LEFT)))
	{
		if (!IsPlayerInPlane (playerid) && GetVehicleModel (GetPlayerVehicleID (playerid)) != 432)//Ignore planes and rhinos.
		{
			...
			if (weaponid != 0 && ammo != 0)//Check if player has got an SMG. Don't punish innoncent players.
			{
				...
				SendClientMessage (playerid, ANTICHEAT, "(Anticheat):{FFFFFF} Molimo vas prestanite raditi drive-by!");
				TogglePlayerControllable (playerid, false);
			...
			case DIALOG_CFG:
			{
				if (response)
				{
					switch (listitem)
					{
						...
						{
							ConfigJunkBuster ();
							SendClientMessage (playerid, JB_GREEN, "(Anticheat):{FFFFFF} Configuration has been loaded from file.");
							ShowPlayerConfigDialog (playerid);
						}
						...
						{
							SaveJunkBusterVars ();
							SendClientMessage (playerid, JB_GREEN, "(Anticheat):{FFFFFF} Configuration has been saved to file.");
							ShowPlayerConfigDialog (playerid);
						}
						...
						{
							JB::Variables = JB::DefaultVariables;
							SendClientMessage (playerid, JB_GREEN, "(Anticheat):{FFFFFF} Default configuration has been loaded.");
			...
			case DIALOG_SETVAR .. (DIALOG_SETVAR + MAX_JB_VARIABLES - 1):
			{
				if (response)
				{
					...
					else
					{
						...
						JB::SendFormattedMessage (playerid, JB_GREEN, "(Anticheat):{FFFFFF} JunkBuster variable '%s' has successfully been set to %d.", JB::VariableNames [var], JB::Variables [var]);
